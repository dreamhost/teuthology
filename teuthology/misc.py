...
def get_scratch_devices(remote):
    ...
    for dev in devs:
        ...
        try:
            rofile = get_file(remote, "/sys/block/%s/ro" % (sysfsdev, )).strip()
            if rofile is '1':
                devs.remove(dev)
        ...
        if len(blkid_stderr) > 0 or r.exitstatus != 0:
            devs.remove(dev)
            log.warn("Removing device due to blkid failure: %s: %s" % (dev, blkid_stderr))
            continue

        dev_blkid_data[dev] = dict([_.strip().split('=', 2) for _ in r.stdout.getvalue().split('\n')])
        ...
        # - vSphere
        # - OVF
        if 'TYPE' in dev_blkid_data[dev] and dev_blkid_data[dev]['TYPE'] is 'iso9660':
            devs.remove(dev)
        ...
        if 'LABEL' in dev_blkid_data[dev] and dev_blkid_data[dev]['LABEL'] in ['config-2', 'cidata', 'CONTEXT', 'CDROM']:
            log.warn("Removing cloud-init device by label: %s" % (dev, ))
            devs.remove(dev)

        # cloud-init, part2:
        # filenames on a volume
        # TODO: this requires mounting the filesystem, which we are not doing now
        pass
